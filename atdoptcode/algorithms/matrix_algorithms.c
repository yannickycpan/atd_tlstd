...
const static struct{
      ...
      { "ATD2nd-FullA", ATD_2ndorder_fullA, update_mat_sherman, update_trace_replacing},
...
void init_matrix_alg(struct alg_t * alg, const int numobservations){
      ...
      mvar_params.r = numfeatures/2<50?(int)(numfeatures/2):50;
      ...
      if (strcmp(alg->name, "TLSTD") == 0) {
         ...
         vars->mvars = allocate_matrix_vars(mattype, numfeatures, &mvar_params);
          ...
          // vars->mvars->t = 0;
	  vars->trueA = gsl_matrix_calloc(numfeatures, numfeatures);
...
int ATD_2ndorder_trueA(void * alg_vars, const struct alg_params_t * params, const struct transition_info_t * info){
    ...
    if(vars->mvars->t==0){
       int size = vars->e->size;
       gsl_permutation *p = gsl_permutation_alloc(size);
       int s;
       // Compute the LU decomposition of this matrix
       gsl_linalg_LU_decomp(vars->trueA, p, &s);
       // Compute the  inverse of the LU decomposition
       gsl_matrix *inv = gsl_matrix_alloc(size, size);
       gsl_linalg_LU_invert(vars->trueA, p, inv);
       gsl_permutation_free(p);
       gsl_matrix_memcpy(vars->trueA, inv);
       gsl_matrix_free(inv);
       gsl_matrix_print(vars->trueA);
     }
    vars->mvars->t++;
    //update z vector
    vars->update_trace(vars->e, params, info);
    ...
    // stepsize = stepsize > params->alpha_t ? stepsize : params->alpha_t;
    // printf("stepsize is %f", params->alpha_t);
    gsl_blas_dgemv (CblasNoTrans, delta*params->alpha_t, vars->trueA, vars->e, 1.0, vars->w);
